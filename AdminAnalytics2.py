...
class Analytics:
    def __init__(self):
        ...
        self.add_bar_graph("Total order per month")
        self.add_line_graph("Total orders per day")

    def get_food_total_data(self):
        query = """ Select EXTRACT(YEAR_MONTH from OrderDate) as OrderDate, FoodOrderID 
                    from world.foodorder
                    order by OrderDate
                """
        result = DatabaseHelper.get_all_data(query)

        df1 = DataFrame(result[1:], columns=result[0]).groupby("OrderDate").count()
        return df1
    ...
    def add_bar_graph(self, title):
        ...
        df1.plot(kind='bar', subplots=True, legend=True, ax=ax1, fontsize=10)
        ax1.set_title(title)
